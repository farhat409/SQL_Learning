...
select s.*
from Sales_order s
INNER JOIN customers c -- joining second table
	on c.customer_id = s.customer -- Join condition
where coalesce(country,'x') <> 'USA'
and s.status = 'In Process';


select coalesce('farhat','1');
select coalesce(null,'farhat','1');
select coalesce(null,null,'farhat','1');
select coalesce(null,null,null,'1','imam');
...
select deal_size -- , avg(sales) as avg_sales  -- WORKS
from sales_order 
group by deal_size;	

select avg(sales) as avg_sales -- WORKS
from sales_order 
group by deal_size;
	
select deal_size , avg(sales) as avg_sales, status -- DOES NOT WORK
from sales_order 
group by deal_size;

select deal_size , avg(sales) as avg_sales, status -- WORKS
from sales_order 
...
and status = 'Shipped'
and order_date between to_date('01-08-2002','dd-mm-yyy')and to_date('31-12-2003','dd-mm-yyyy');
...
where p.product_line in ('Trains','Planes','Ships')
...
select *
from Sales_order
where order_date between to_date('01-02-2002','mm-dd-yyyy') and to_date('01-05-2023','mm-dd-yyyy')
and quantity_ordered % 2 =0;

8) Find the average sales order price

select round(avg(sales)) as avg_sales_order
from Sales_order
...
select count(quantity_ordered) as total_order
from sales_order;
